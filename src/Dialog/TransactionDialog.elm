...
init initType store =
    let
        ...
        getDialogData isDirty transactionData =
            let
                notDeletedTransactionDataList =
                    Transaction.getNotDeletedTransactionDataList store.transactions
            ...
            case initType of
                ...
                New ->
                    let
                        ...
                        transactionData =
                            Transaction.getNewTransactionTemplate store.transactions uuid
                    in
                    ( NewTransaction (getDialogData False transactionData), newS )
    in
    ( { store = newStore, dialog = dialog }
...
viewTransactionForm : DialogData -> Dialog -> Model -> String -> Html Msg
viewTransactionForm dialogData dialog model leftButtonText =
    let
        ...
        buttons =
            div [ c "buttons" ]
                [ b { onClick = DeleteClicked, id = "deleteButton", text = leftButtonText }
                , b { onClick = SaveClicked, id = saveButtonId, text = "Save" }
                ]
